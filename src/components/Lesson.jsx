...
export default function Lesson({ moduleId }) {
    ...
    const handleComplete = () => {
        if (!state.lessons[mod.id]) {
            updateLessonStatus(mod.id, true);
            setNotification({
                type: 'success',
    ...
    // Check if we should render the subnet calculator (usually in networking lessons)
    const showSubnetCalc = mod.lessonHTML.includes('id="subnet-calculator-container"') || moduleId === 'mod2';
    ...
    return (
        <div className="lesson-container">
            ...
            <article className="lesson-content" aria-label="Contenu de la leçon" dangerouslySetInnerHTML={{ __html: mod.lessonHTML }} />
            ...
            {notification && (
                <div className={`lesson-notification lesson-notification--${notification.type}`} role="alert" aria-live="assertive">
                    ...
                    {notification.type === 'success' && (
                        <a href={`/${mod.id}/quiz`} className="btn btn-primary btn-sm" style={{ marginLeft: 'var(--space-4)', textDecoration: 'none' }}>
                            Aller au Quiz →
                        </a>
                    )}
                </div>
